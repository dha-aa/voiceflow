...
The Models pane places its speech-engine controls and model content inside the pane’s scrollable region so long diagnostics, validation messages, and model rows do not overflow the fixed Settings window. It exposes a provider-level **Speech recognition** picker backed by `SpeechRecognitionSettings`. The picker presents **WhisperKit** and **FluidAudio**; Parakeet v2/v3 are FluidAudio models, not separate speech engines. The persisted choices are `WhisperKit` (default) and the FluidAudio engine (internally represented by the existing Parakeet route). A second picker appears when FluidAudio is selected and persists `ParakeetModelVariant.v3` or `.v2` through `ParakeetModelManager`. Changing the speech engine or FluidAudio model variant updates the shared `SpeechTranscriptionRouter` and preload lifecycle; it does not alter the Claude/ChatGPT AI-provider setting.
...
**Download** invokes `ParakeetModelManager`, uses FluidAudio’s repository-aware download path, reports progress, and exposes **Cancel** while the operation is active. Download state is manager-owned, so switching Settings tabs does not cancel or erase it. Cancellation propagates to the underlying task; the UI shows **Cancelling…** until cleanup finishes, then refreshes the selected variant without marking it installed. The manager structurally checks the exact required artifacts, checks compiled bundle contents, and performs a FluidAudio load validation before marking the variant installed. A partial, malformed, upstream-source, or load-failing folder remains uninstalled. The normal **Download** action automatically removes the selected incomplete folder when repair is needed, then downloads the supported conversion again. **Open Folder** opens the selected variant’s canonical directory in Finder.
...
- The Models pane remains usable at the fixed Settings size because the speech-engine controls, validation diagnostics, and model rows scroll instead of expanding the window.
- A FluidAudio model download shows progress and a visible Cancel action; cancellation returns to a retryable not-installed state and never claims success.
- A Download from an incomplete FluidAudio cache automatically recreates only the selected Parakeet cache folder and never touches WhisperKit models.
