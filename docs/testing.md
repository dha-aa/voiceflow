...
The current automated baseline is **194 XCTest tests with zero failures**. Tests that require microphone, Accessibility, a live WhisperKit model, or another application are supplemented by manual verification rather than being made dependent on a particular user machine.
...
```text
Executed 194 tests, with 0 failures
** TEST SUCCEEDED **
```
...
| Scenario | Procedure | Expected result |
|---|---|---|
| Native editable control | Focus a TextEdit document or another native editable field, optionally select text, and complete dictation. | While the captured target remains frontmost, VoiceFlow copies the result to a temporary clipboard and automatically posts Command-V. If that route fails, Accessibility value replacement is attempted, followed by captured-process keyboard fallback. The selected UTF-16 range is replaced when AX exposes it, the caret is placed after the inserted text when supported, and the prior clipboard is restored. |
...
| Browser textarea/contenteditable | In Safari, Chrome, or Firefox, focus a disposable textarea or `contenteditable` editor and complete dictation. | While the captured browser remains frontmost, VoiceFlow first writes temporary clipboard contents and automatically posts Command-V. If the control rejects that route, AX value mutation or captured-process keyboard fallback may be attempted, then the previous clipboard is restored. Record the exact browser/editor result; do not generalize it to all web controls. |
...
| Clipboard preservation | Put a disposable marker in the clipboard, test a frontmost non-terminal copy-then-Command-V route and a terminal paste, then inspect the clipboard afterward. | The marker remains available after the pasteboard propagation delay and temporary paste, including after clipboard-write, paste-command, or caret-command errors where the system permits observation. |
...
| Valid text target | Focus an editable TextEdit field and complete dictation without changing focus after Fn-down. | VoiceFlow writes the result temporarily, automatically posts **⌘ + V** into the captured frontmost field, restores the prior clipboard, and shows **Done!** when the target accepts the paste. It must not require the user to press Command-V manually. |
